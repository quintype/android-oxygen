...
public class StoryElement implements Parcelable {
    ...
    public static final String TYPE_GALLERY_ADS = "type_gallery_ads"; // Dummy type specifically
    // for 'TheQuint'
    ...
    private boolean isTypeJsEmbedWithTwitter = false;
    ...
    StoryElement(StoryElement element) {
        ...
        this.attachmentFileName = element.attachmentFileName;
    ...
    public boolean isTypePollType() {
        ...
        if (type.equalsIgnoreCase(TYPE_STORY_ELEMENT_POLLTYPE)) {
            ...
            if (TextUtils.isEmpty(decodedJsEmbed)) {
                ...
                String decodedJsEmbed = new String(data, UTF_8);
    ...
    public boolean isReferences() {
        if (subType != null)
            return isTypeComposite() && subType.equalsIgnoreCase(SUB_TYPE_STORY_ELEMENT_REFERENCES);
        else return false;
    }

    public boolean isIconWithTitle() {
        return TYPE_ICON_WITH_TITLE.equals(type);
    }
    ...
    public boolean isAttachment() {
        if (type != null && type.equalsIgnoreCase(TYPE_STORY_ELEMENT_FILE))
            return subType != null && subType.equalsIgnoreCase(SUB_TYPE_STORY_ELEMENT_ATTACHMENT);
        else return false;
    }
    ...
    public boolean isGalleryAds() {
        return type.equalsIgnoreCase(TYPE_GALLERY_ADS);
    }
    ...
    private void isJsEmbedTwitterElement() {
        if (!TextUtils.isEmpty(embedJs)) {
            try {
                byte[] data = Base64.decode(embedJs, Base64.DEFAULT);
                decodedJsEmbed = new String(data, StandardCharsets.UTF_8);
    ...
    public boolean isSocialShareWidget() {
        return TYPE_SHARE_WIDGET.equals(type);
    }
